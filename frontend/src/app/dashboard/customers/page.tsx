...
export default function CustomersPage() {
  ...
  const validate = () => {
    ...
    if (!editId && customers.some((c) => c.dni === formData.dni))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editId) {
      saveCustomers(
        customers.map((c) =>
          c.id === editId
            ? { ...c, name: formData.name.trim(), email: formData.email.trim(), phone: formData.phone.trim() }
            : c
        )
      );
      setEditId(null);
      ...
      saveCustomers([
        { id: crypto.randomUUID(), name: formData.name.trim(), dni: formData.dni, email: formData.email.trim(), phone: formData.phone.trim(), createdAt: new Date().toISOString() },
        ...customers,
      ]);
    }
  ...
  const handleDelete = (c: Customer) => {
    ...
    saveCustomers(customers.filter((item) => item.id !== c.id));
  };
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-gray-100">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <button type="submit" className="self-end rounded-md bg-pink-600 px-4 py-2 text-sm font-bold text-white hover:bg-pink-700">
